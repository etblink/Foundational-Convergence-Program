...
| `FW-CAT` | Broader categorical / process-theoretic / topos / related structural approaches | comparator family pending subdivision/source intake | `REMOVED_WITH_REASON` | FW-CAT Taxonomy Gate Stage 2 selects verdict B: the frozen 32-source corpus resolves into 12 objects, of which four map to existing `FW-CQM`, `FW-GPTOPT`, `FW-AQFT`, or the GR sector represented by `FW-NULL-GRQFTSM`, six remain nonframework mathematical, representational, axiomatic, classificatory, or model-toolkit material, and two physically motivated proposals remain deferred; no retained or successor framework ID is admitted, while all source-qualified content is preserved at its actual scope |
