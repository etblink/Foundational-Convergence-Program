...
| `FW-PROCESS-MATRIX` | Process-matrix operational framework | source-bound generalizing causal-process meta-framework | `ADMITTED_NOT_AUDITED` | Dedicated causal-process / ICO Stage-2 taxonomy under Method 0.2.1 admits the generalized process-matrix object after A–H all pass. Its core is local quantum laboratories/operations plus a global process object constrained to yield valid probabilities without assuming one fixed global causal order. The framework is not identical to ICO, causal nonseparability, causal-inequality violation, quantum switch/QC-QC, resource theory, experimental implementation, or QG; no K1–K10 baseline, framework-level empirical selection, pairwise comparison, convergence credit, or recurrence result is yet assigned |
