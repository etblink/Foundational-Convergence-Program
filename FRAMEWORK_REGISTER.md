...
| `FW-AS` | Asymptotic safety | source-bound continuum/RG fixed-point quantum-gravity comparator | `PAIRWISE_COMPARISON_COMPLETE` | Targeted source strengthening partially closes the physical/Lorentzian-observable gap: selected global UV→IR recovery and Lorentzian spectral realization qualify at E3-M, and a timelike model observable is source-qualified with positive viability content. The mandatory uncertainty/tolerance predicate remains unqualified for S1 E4, so `AS_S1_E4 = UNRESOLVED_UNDER_FROZEN_CORPUS`; E3-F/E3-P and EMP4 remain unestablished. Historical FCP-21 remains the latest Reduced-NFC/AS comparison |
