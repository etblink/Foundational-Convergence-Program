...
| `FW-CAT` | Broader categorical / process-theoretic / topos / related structural approaches | historical umbrella / provenance only | `REMOVED_WITH_REASON` | FW-CAT Stage 2 umbrella removal and verdict B remain current, with the post-audit `OBJ-CAT-11` correction applied: the frozen 12 objects now yield three existing-framework assignment objects (`OBJ-CAT-01`→`FW-CQM`, `OBJ-CAT-06`→`FW-AQFT`, `OBJ-CAT-12`→the GR sector of `FW-NULL-GRQFTSM`), six nonframework remainders, and three deferred proposals (`OBJ-CAT-08`, `OBJ-CAT-09`, `OBJ-CAT-11`). `OBJ-CAT-11` process-matrix / indefinite-order content is distinct from existing GPTOPT/CQM identity but remains unadmitted; no successor framework ID is created |
